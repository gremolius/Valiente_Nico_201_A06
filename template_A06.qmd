...
<https://github.com/gremolius/Valiente_Nico_201_A06>
...
for (i in names(player_info)) {
  player_info[[i]]$height <- heights[index]
  index <- index + 1
}
...
mat_rand <- apply(mat_rand, MARGIN = 1:2, q3)
mat_rand
```
...
```{r}
col_means <- max_temps %>% 
  summarize(across(c(jan:dec), mean))
```
...
```{r}
colorado_median <- colorado %>% 
  group_by(year) %>%
  summarize(median = median(c_across(jan:dec)))

max_median_year <- colorado_median$year[which.max(colorado_median$median)]
min_median_year <- colorado_median$year[which.min(colorado_median$median)]

max_median <- colorado_median$median[which.max(colorado_median$median)]
min_median <- colorado_median$median[which.min(colorado_median$median)]

print(paste0("The maximum median annual temperature of ", max_median, " occurred in ", max_median_year,  " and the minimum median annual temperature of ", min_median, " occurred in ", min_median_year, "."))
```
...
sum_of_numbers <- function(n) {
  if (n == 0) {
    ...
    return((n %% 10) + sum_of_numbers(floor(n / 10)))
  }
}

example <- sum_of_numbers(935068)
print(example)
```
